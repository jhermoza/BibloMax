...
{
    public class RijndaelUtilitario
    {
        ...
        public static string EncryptRijndaelToUrl(string text)
        {
            return HttpUtility.UrlEncode(EncryptRijndael(text, LLAVE_DEFECTO));
        }

        public static string EncryptRijndaelToBase64(string text)
        {
            var plainTextBytes = System.Text.Encoding.UTF8.GetBytes(EncryptRijndael(text, LLAVE_DEFECTO));
            return System.Convert.ToBase64String(plainTextBytes);
        }

        public static T DecryptRijndaelFromBase64<T>(string base64)
        {

            try
            {
                var base64EncodedBytes = System.Convert.FromBase64String(base64);
                var cifrado = System.Text.Encoding.UTF8.GetString(base64EncodedBytes);
                return DecryptRijndael<T>(cifrado);
            }
            catch
            {
                return default(T);
        ...
        public static T DecryptRijndael<T>(string cifrado)
        {
            ...
            try
            {
                resultado = (T)Convert.ChangeType(DecryptRijndael(cifrado, LLAVE_DEFECTO), typeof(T));
            }
            catch
            {
                resultado = default(T);
